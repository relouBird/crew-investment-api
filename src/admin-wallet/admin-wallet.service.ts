...
import { properAdminTxComposable, properTxComposable } from 'src/utils/mapper';
...
export class AdminWalletService {
  ...
  async getStatisticWalletByUserId(uid: string) {
    ...
    return properAdminTxComposable(transactions);
  }
  ...
  async checkState(transactionId: number) {
    ...
    const depositTrans = await this.transactionService.findAllByStatusAndType(
      'done',
      'deposit',
    );
    const withdrawTrans = await this.transactionService.findAllByStatusAndType(
      'done',
      'withdrawal',
    );
    ...
    depositTrans.forEach((d) => {
      funds += d.amount;
    });

    withdrawTrans.forEach((d) => {
      funds -= d.amount;
    });
  ...
  /**
   * Faire un retrait sur un compte Admin
   */
  async withdrawUserAccount(uid: string, data: WithdrawalWalletDto) {
    const user = await this.getUserLocaly(uid);

    const description = `Retrait Admin - ${data.service}`;
